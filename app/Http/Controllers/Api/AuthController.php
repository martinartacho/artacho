...
public function login(Request $request)
{
    ...
    // ➕ Enviar notificación tipo 'welcome' si aún no la recibió
    try {
        $yaTiene = DB::table('notification_user')
        ...
        if (!$yaTiene) {
            ...
            $body = 'Gracias por unirte. Esperamos que disfrutes de todas las funcionalidades.';
            ...
            app(\App\Services\FCMService::class)->sendToUser($user, [
                'title' => $title,
                'body' => $body,
            ], [
                'type' => 'welcome',
            ...
            Log::info('[AuthController] Notificación de bienvenida enviada', [
                'user_id' => $user->id,
                'notification_id' => $notification->id
            ]);
        }
    } catch (\Exception $e) {
        Log::error('[AuthController] Error al enviar notificación de bienvenida', [
            'user_id' => $user->id,
            'error' => $e->getMessage()
        ]);
    }
